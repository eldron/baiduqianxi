...
r = os.listdir("moveout_index20192020")
print(len(r))
files = []
for item in r:
	if os.path.isfile("moveout_index20192020/" + item):
		files.append(item)
print('number of files = ' + str(len(files)))

workbook = xlsxwriter.Workbook(u"人口迁出指数(2.1).xlsx")
...
for filename in files:
	...
	f = open("moveout_index20192020/" + filename, "r")
	s = f.read()
	f.close()
	...
	try:
		for date, value in dic.items():
			...
			#cityname = cityname.decode('gbk')
			if cityname not in provinces and cityname not in autonomous:
				if cityname == u'吉林市' or "县" in cityname or "地区" in cityname:
