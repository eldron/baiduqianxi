...
#http://huiyan.baidu.com/migration/provincerank.jsonp?dt=city&id=420100&type=move_in&date=20200218
...
if __name__ == '__main__':
	if len(sys.argv) != 5:
		print(f"usage: {sys.argv[0]} type startdate enddate directory")
	else:
		type = sys.argv[1]
		...
		partone = "http://huiyan.baidu.com/migration/provincerank.jsonp?dt=city&id="
		parttwo = "&type="
		partthree = "&date="
		for date in days:
			for id, name in cdic.items():
				url = partone + id + parttwo + type + partthree + date
				print(f"url is {url}")
				if type == "move_in":
					filename = "迁入省份比例." + name + '.' + date + '.html'
				else:
					filename = "迁出省份比例." + name + '.' + date + '.html'
