...
if __name__ == '__main__':
	if len(sys.argv) != 3:
		print(f'usage: {sys.argv[0]} htmldir output_filename')
	else:
		htmldir = sys.argv[1]
		if '/' not in htmldir:
			htmldir += '/'
		...
		cities = []
		for name, id in nametoid.items():
			if name not in provinces and name not in autonomous:
				cities.append(name)

		r = os.listdir(htmldir)
		print(len(r))
		files = []
		for item in r:
		...
		worksheet.write(0, 0, u"城市")
		worksheet.write(0, 1, u"日期")
		worksheet.write(0, 2, u"迁入省份" if 'movein' in htmldir else '迁出省份')
		worksheet.write(0, 3, u"迁入城市" if 'movein' in htmldir else '迁出城市')
		worksheet.write(0, 4, u"百分比")
		row = 1
		for city, data_per_day in dic.items():
			for date, detailed_data in data_per_day.items():
				#worksheet.write(row, 0, date)
				for item in detailed_data:
					worksheet.write(row, 0, city if '市' in city or '县' in city or '地区' in city else city + '市')
					worksheet.write(row, 1, date)
					worksheet.write(row, 2, item[u'province_name'])
					worksheet.write(row, 3, item[u'city_name'])
					worksheet.write(row, 4, item[u'value'])
					row += 1
		workbook.close()